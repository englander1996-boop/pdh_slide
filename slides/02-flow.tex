...
\begin{frame}
  ...
  \begin{tikzpicture}[
    ...
    \node[io, font=\LARGE\bfseries] (fresh) at (0, 0) {原料\\LPG};
    \node[pump]  (P)     at (1.6,  0)    {P};
    \node[col]   (D1)    at (3.6,  0)    {脱\\ブタン塔};
    \node[io, below=12mm of D1] (waste) {$\mathrm{C_4}$ 留分};
    \coordinate  (V)     at (6.0,  0);
    \node[circle, fill=black, inner sep=0pt, minimum size=4.5mm] (J) at (7.6, 0) {};
    ...
    \node[io, font=\LARGE\bfseries] (h2)  at (19.7,-1.8) {$\mathbf{H_2}$\\製品};
    ...
    \node[io, font=\LARGE\bfseries] (prod) at (20.0,-5.0) {$\mathbf{C_3H_6}$\\製品};
    ...
    % ===== リサイクル（膜・C3：各ユニット直下に膨張弁→直角配線でミキサー J へ）=====
    %  斜め線を避け、各ユニット真下に膨張弁を置き、バス→垂直に大黒丸ミキサーへ入れる。
    \coordinate (Vm) at (12.8,-10.4);  % 膜分離器の真下
    \coordinate (Vc) at (18.3,-11.0);  % C3 スプリッタ塔の真下
    \draw[flow] (MEM.south) -- (Vm) -- (12.8,-11.2) -| (7.8,0);  % 膜非透過→真下弁→バス→垂直にミキサー
    \draw[flow] (D3.south)  -- (Vc) -- (18.3,-11.9) -| (7.4,0);  % C3塔底→真下弁→バス→垂直にミキサー
    \node[io, anchor=south] at (14.6,-11.45) {リサイクル};
    \node[anchor=south, font=\large] at ([yshift=2.6mm]J) {ミキサー};
    % ===== 用いた主要仮定（左下の空きスペースを縦いっぱいに活用・見出しなし）=====
    \node[anchor=north west, align=left, draw=HeaderBlue, line width=1pt,
          rounded corners=3pt, inner sep=6pt, text width=8.3cm, font=\Large,
          fill=white] at (-1.95,-3.75)
      {・スイングは時間平均で定常とみなし\\
       　リサイクルは収束計算で閉じる\\[0.5em]
       ・物性は Peng--Robinson 状態方程式\\
       　反応器は低圧高温で理想気体近似\\[0.5em]
       ・触媒活性は時間と温度で低下し\\
       　主反応のみ・床内最高温で代表\\[0.5em]
       ・反応器は断熱・1次元の流れ\\
       　圧力損失も同時に連成計算\\[0.5em]
       ・蒸留は平衡段・一定モル流れ\\
       　段効率 $0.8$・段間隔 $0.6$ m\\[0.5em]
       ・原料 $\mathrm{C_3H_8}{:}\mathrm{C_4H_{10}}=9{:}1$\\
       　製品 $\mathrm{C_3H_6}$ $99.5$ wt\%・$\mathrm{H_2}$ $99.9$ mol\%};

    % ===== 膨張弁（蝶ネクタイ記号）：脱ブタン塔頂(V)・膜(Vm)・C3(Vc) の3か所 =====
    ...
    \node[lbl, anchor=west] at ([xshift=2.8mm]Vm) {膨張弁};
    \node[lbl, anchor=east] at ([xshift=-2.8mm]Vc) {膨張弁};
  \end{tikzpicture}%
  }
  \end{center}

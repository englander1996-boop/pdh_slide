...
\newlength{\descIndent}\setlength{\descIndent}{0.6mm}
...
\begin{frame}[t]
  ...
  \vspace*{1.2em}   % 全体を少し下げ、r1 の分数上端（K）を帯から離す
  % ===== 上：反応ネットワーク + 速度式（全幅）======================
  {\normalsize
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.74\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          font=\scriptsize,
          width=\dimexpr\linewidth-\grShrink-8mm\relax, height=5.08cm,
      ...
      \centering
      \vspace*{-0.92em}
      {\scriptsize 　　　　　触媒活性 $\actA$ の経時低下（温度別）}
    \end{column}
  \end{columns}

  \vspace*{0.2em}
  \noindent{\tiny 速度式パラメータ・活性データの出典：第 17 回プロセスデザイン学生コンテスト要項}

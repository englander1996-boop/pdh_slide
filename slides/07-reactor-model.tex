...
\begin{frame}[t]
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.49\textwidth}
      ...
      \vspace{0.45em}
      {\footnotesize 再生追従のセット数}\par
      {\centering$\displaystyle N_{\mathrm{swing,sets}}=\big\lceil t_{\mathrm{regen}}/t_{\mathrm{cyc}}\big\rceil+1$\par}
      \vspace{0.5em}
      {\footnotesize 触媒体積}\par
      {\centering$\displaystyle V_{\mathrm{cat}}=(1-\varepsilon)\,z_{\mathrm{cat}}\,\pi D^2/4$\par}
      \vspace{0.5em}
      {\footnotesize 容器分割の並列基数}\par
      {\centering$\displaystyle N_{\mathrm{parallel}}=\max\!\big(\lceil V_{\mathrm{cat}}/V_{\mathrm{cat,max}}\rceil,\,1\big)$\par}
      \vspace{0.5em}
      {\footnotesize 容器1基の体積　CAPEX 基準}\par
      {\centering$\displaystyle V_{\mathrm{vessel}}=z_{\mathrm{cat}}\,\pi D^2/(4\,N_{\mathrm{parallel}})$\par}
      \vspace{0.55em}
      {\color{black!25}\rule{\linewidth}{0.4pt}}\par
      \vspace{0.45em}
      % --- 右表：スイング設計の記号 2群 ---

...
%   どのペアでも最小 TAC の谷が斜め＝2変数の最適値が互いに依存 → 全 22 変数を同時最適化。
...
\begin{frame}[t]
  ...
  \vspace*{2.0ex}
  {\bfseries\normalsize 最適値が相互依存　$\to$　全 22 変数を同時最適化}\par

  \vspace{0.6ex}
  \centering
  \includegraphics[width=0.95\linewidth]{fig_interaction_panels}\par
  \vspace{0.2ex}
  {\scriptsize 簡易モデルで検証：各ペアの TAC 等高線（☆：採用設計・灰：制約違反・{\color{SpRed}赤線}：各列の TAC 最小点）}\par
  \vspace{0.25em}

...
%   分岐56.0円/kg・現行95円/kgで赤字290億円/年）。結言文=指定どおり。
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.50\textwidth}
      ...
      {\small\raggedright
        ...
        \vspace{0.45em}
        損益分岐点は \textbf{LPG $56.0$ 円/kg}。現行 $95$ 円/kg では {\color{SpRed}\textbf{赤字 $290$ 億円/年}}。\par
        \vspace{0.45em}
        $\to$ \textbf{安価な原料が得られる立地なら黒字化}。\par}
      \vspace{0.8em}
      {\fontsize{5.6}{6.6}\raggedright\selectfont
       現行想定 $95$ 円/kg は CP プロパン約 $600$ USD/t 相当。\par
       出典 Argus Media, LPG Outlook 26-2, 2026\par}
    \end{column}
    \begin{column}{0.50\textwidth}
      \centering
      \includegraphics[width=0.92\linewidth]{breakeven_slide}
    \end{column}
  \end{columns}
  ...
  {\setlength{\fboxsep}{6pt}\setlength{\fboxrule}{1.2pt}%
   \fcolorbox{HeaderBlue}{HiBlue!10}{%
    \begin{minipage}{\dimexpr\linewidth-16pt}
     ...
     {\small LPG を プロパン脱水素化でプロピレン {\boldmath$402$} kt/年・{\boldmath$99.5$} wt\% に転換する全プロセスを設計・最適化した。採算性は原料 LPG の調達コストに支配され、\textbf{本プロセスは安価な LPG を確保できる立地でこそ成立する}。}
    \end{minipage}}}

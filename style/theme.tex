...
\setbeamersize{text margin left=1mm, text margin right=1mm}  % 本文を横幅ぎりぎりまで

% 【必須ルール】帯と本文の間の余白を詰める（全スライド共通）。
%  beamer 既定の天井スキップで本文が帯から離れるのを headline 直後の負スキップで打ち消し、
%  帯下端→本文先頭の隙間を約 1mm に調整（300dpi 実測で 1.0mm、_drvgap.tex で検証）。
%  -2.6ex で既定スキップを相殺し、追加の -2.9mm で 1mm まで詰める。
%  情報量最優先：無駄な上余白は作らない。各スライドは [t] 推奨。
\addtobeamertemplate{headline}{}{\vskip-2.6ex\vskip-2.9mm}
